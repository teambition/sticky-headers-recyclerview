...
public class HeaderPositionCalculator {
    ...
    public boolean hasNewHeader(int position, boolean isReverseLayout) {
        ...
        if (mAdapter instanceof StickyRecyclerHeadersGridAdapter) {
            StickyRecyclerHeadersGridAdapter gridAdapter = ((StickyRecyclerHeadersGridAdapter) mAdapter);
            int numColumns = gridAdapter.getNumColumns();
            int columnOfItems = position % numColumns;
            if (columnOfItems > 0) {
                int firstItemOnRowPosition = position - columnOfItems;
